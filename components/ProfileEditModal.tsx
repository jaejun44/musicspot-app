...
export default function ProfileEditModal({ user, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    supabase
      .from('user_profiles')
      .select('display_name, bio, instruments, genres, region, purposes, avatar_url')
      .eq('user_id', user.id)
      .maybeSingle()
      .then(({ data }) => {
        const fallbackName =
    ...
    await supabase.from('user_profiles').upsert(
      {
        ...
        is_public: instruments.length > 0,
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-[#0A0A0A]/60 flex items-end sm:items-center justify-center"
    ...
    >
      <motion.div
        ...
        className="w-full sm:max-w-lg bg-[#FFF8F0] rounded-t-[24px] sm:rounded-[24px] border-[3px] border-[#0A0A0A] max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        <div className="sticky top-0 bg-[#FFF8F0] px-5 pt-5 pb-3 border-b-[2px] border-[#0A0A0A]/10 z-10">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="w-9 h-9 rounded-full bg-white border-[2px] border-[#0A0A0A] flex items-center justify-center text-[16px] font-bold"
          ...
          <section className="flex flex-col items-center gap-3">
            <div className="relative">
              ...
              <motion.button
                ...
                className="absolute -bottom-1 -right-1 w-7 h-7 bg-[#FF3D77] rounded-full border-[2px] border-[#0A0A0A] flex items-center justify-center text-[13px]"
