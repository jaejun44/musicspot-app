...
function mapMicError(e: unknown): string {
  ...
  switch (err?.name) {
    ...
    case 'SecurityError':
      ...
      return t('마이크 접근이 거부됐어요. ① 주소창 왼쪽 자물쇠 → 마이크 → 허용, ② Mac은 시스템 설정 → 개인정보 보호 및 보안 → 마이크에서 브라우저 ON(켠 뒤 브라우저 완전 재시작) 후 다시 시도해주세요.');
    case 'NotFoundError':
    case 'OverconstrainedError':
      return t('사용할 마이크 장치를 찾지 못했어요. 마이크가 연결돼 있는지 확인해주세요.');
    case 'NotReadableError':
      return t('마이크를 다른 앱/탭이 쓰고 있어요. 줌·녹음기·다른 탭 등을 끄고 다시 시도해주세요.');
    case 'AbortError':
      return t('마이크를 여는 중 중단됐어요. 다시 시도해주세요.');
    default:
      return t('마이크를 열 수 없어요 ({name}: {message}).', { name: err?.name || t('알 수 없는 오류'), message: err?.message || '' });
  }
}
...
async function micDiagnostics(): Promise<string> {
  ...
  const browser = /Edg\//.test(ua)
    ...
    ? 'Safari'
    : t('기타');
  return t('[진단] 권한={perm} · 마이크={mics}개 · {browser}', { perm, mics, browser });
}
  ...
  if (typeof navigator === 'undefined' || !navigator.mediaDevices?.getUserMedia) {
    return {
      error: isInsecureContext()
        ? t('이 주소({host})에선 마이크를 못 써요. 브라우저는 HTTPS 또는 localhost에서만 마이크를 허용합니다. https:// 주소나 localhost로 접속해주세요.', { host: window.location.host })
        : t('이 브라우저에서 마이크 기능(getUserMedia)을 지원하지 않습니다.'),
    };
  }
  try {
