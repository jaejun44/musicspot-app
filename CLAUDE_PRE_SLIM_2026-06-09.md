...
**원칙: 기본은 작게, 필요하면 키운다.** "무조건 작게"가 아니라 "목적에 맞는 만큼"이 기준. 탐색·파악 단계는 가볍게, 진짜 데이터가 필요한 분석·디버깅 단계는 충분히 받되 목적을 밝히고 범위를 넓힌다.

### Supabase MCP
- **스키마 파악**: `list_tables`로 전체 스키마를 통째로 받지 말 것 (응답 수만 토큰). 특정 테이블 구조만 필요하면 `execute_sql`로 `information_schema.columns`를 해당 테이블로 한정 조회.
- **데이터 조회**: `SELECT *` 금지. 필요한 컬럼만 명시 + `LIMIT` 기본 부착. 샘플 파악은 `LIMIT 5~10`으로 시작.
- **예외**: 디버깅·분석에 더 많은 행/컬럼이 필요하면 → 목적을 한 줄로 밝히고 범위를 넓혀 조회 (질을 위해 필요한 데이터는 받는다).
- ⚠️ 위 "데이터 패턴 > 1,000행 제한 우회"는 앱 런타임 코드용. MCP 탐색 조회와 혼동 금지.

### Vercel MCP
- `list_deployments`: 최신 1~3개만 (`limit` 파라미터 활용).
- `get_logs` / `get_runtime_logs`: 라인 수·시간 범위 제한해서 호출. 전체 로그 덤프 금지.
- `list_projects`: 프로젝트 식별 후엔 재호출 금지 (결과 재사용).

### 공통
- 같은 MCP 결과를 같은 세션에서 반복 호출하지 말 것 — 이미 받은 응답 재사용.
- 큰 응답이 예상되면 먼저 count/구조만 확인 → 정말 필요한 범위만 본조회.
